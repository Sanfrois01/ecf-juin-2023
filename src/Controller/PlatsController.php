...

{
    ...
    public function platsListe( PlatRepository $platRepository , CategoriePlatRepository $categoriePlatRepository, Request $request)
    {
        $data = new SearchData();
        ...
        if($request->get('ajax')){
            return new JsonResponse([
                'content' => $this->renderView('_plats.html.twig',['plats' => $plats])
            ]);
        }
    ...
    public function platDelete ($id , PlatRepository $platRepository , EntityManagerInterface $entityManager)
    {
        ...
        $this->addFlash('success', 'Plats supprimé avec succés');
    }
    ...
    public function platCreate(Request $request , EntityManagerInterface $entityManager) 
    {
        ...
        $this->addFlash('success' , 'Plat créé avec succées');
